...
This port is not starting from zero. The workspace already contains:
...
The important current limitation is that the workspace is still a hybrid baseline, not yet a real Rust libc replacement:
...
Repository reality check: relative to `HEAD`, the prepared `safe/**` workspace is not yet committed in git; `git diff f87362aca3d918f740063d16cd1dd4bd250fd7de..HEAD` is empty and `git status --short --untracked-files=all` reports `safe/` as untracked. The same snapshot currently has no `safe/work/original-build/**` tree, and `safe/scripts/` currently contains only `build-debs.sh` and `install-safe-repo.sh`; there is no `safe/scripts/stage-original-build.sh` yet. The workflow therefore cannot assume that phases 02-05 already exist as reviewable commits or that the staged upstream build prerequisite is already present. This plan adds a landing phase, `impl_05a_commit_prepared_safe_frontier`, that commits the prepared `safe/**` tree first, creates and commits `safe/scripts/stage-original-build.sh`, and uses that helper when needed to recreate only the derived `safe/work/original-build/**` tree locally so the existing phase-04/05 verifiers can run. Every later reference to a "committed" `safe/**` artifact means the post-`impl_05a_commit_prepared_safe_frontier` state.

The prepared tree also currently contains transient Python bytecode caches under `safe/tests/**/__pycache__/` and `safe/tests/**/*.pyc`. Those files are not authoritative source artifacts for the port; phase `05a` must delete any such cache files before landing the baseline, then ignore only those exact cache patterns going forward.

The plan therefore has two simultaneous jobs:

1. Preserve the already-prepared artifact contracts instead of regenerating them.
...
The safest incremental architecture is:
...
That design is the only practical way to preserve runtime and link compatibility while porting `libc.so.6` and related DSOs across multiple linear phases.

## Generated Workflow Contract

The generated workflow must follow these fixed rules:
...
Prepared on-disk frontier before this workflow starts (not yet committed relative to `HEAD`):
...
The remaining linear workflow is `impl_05a_commit_prepared_safe_frontier`, then phases `impl_06_*` through `impl_10_*`.
...
After `impl_10_final_fixup_and_audit` lands, the full implementation is verified only by running the complete end-to-end sequence below. This sequence mirrors the explicit `check_10_*` verifiers above and begins by validating or materializing the ignored staged upstream build tree under `safe/work/original-build/**` through the dedicated phase-06 helper instead of assuming that tree is already present:
