...
validator_image_guard_pid=""
validator_image_backup_tag=""

validator_image_tag_for() {
  local library="$1"
  local image_name
  image_name="$(printf '%s-shared' "$library" | sed 's/[^A-Za-z0-9]/-/g; s/^-*//; s/-*$//')"
    ...
    while :; do
      if docker image inspect "$image_tag" >/dev/null 2>&1; then
        if ! docker image inspect "$backup_tag" >/dev/null 2>&1; then
          docker tag "$image_tag" "$backup_tag" >/dev/null 2>&1 || true
        fi
      elif docker image inspect "$backup_tag" >/dev/null 2>&1; then
        docker tag "$backup_tag" "$image_tag" >/dev/null 2>&1 || true
      fi
      sleep 2
    done
  ) &
  validator_image_guard_pid="$!"
  validator_image_backup_tag="$backup_tag"
}

stop_validator_image_guard() {
  if [[ -n "$validator_image_guard_pid" ]]; then
    kill "$validator_image_guard_pid" >/dev/null 2>&1 || true
    wait "$validator_image_guard_pid" 2>/dev/null || true
    validator_image_guard_pid=""
  fi
  if [[ -n "$validator_image_backup_tag" ]] && command -v docker >/dev/null 2>&1; then
    docker image rm --force "$validator_image_backup_tag" >/dev/null 2>&1 || true
    validator_image_backup_tag=""
  fi
}

trap stop_validator_image_guard EXIT
validator_image_tag="$(validator_image_tag_for "$SAFELIBS_LIBRARY")"
start_validator_image_guard \
  "$validator_image_tag" \
  "${validator_image_tag}-safelibs-keep-${commit_sha:0:12}"
